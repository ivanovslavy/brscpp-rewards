...
contract BRSCPPRewards is ReentrancyGuard, Ownable {
    ...
    event UnclaimedWithdrawn(Category indexed category, uint256 amount, address indexed caller);
    ...
    // ============================================
    // WITHDRAWAL (PUBLIC - SENDS TO DEPLOYER)
    // ============================================
    
    /**
     * @notice Withdraw unclaimed bounty (anyone can call, funds go to deployer)
     * @param _category TECHNICAL or MARKETING
     */
    function withdrawUnclaimed(Category _category) external nonReentrant {
        Bounty storage bounty = bounties[_category];
        ...
        emit UnclaimedWithdrawn(_category, amount, msg.sender);
    }
     ...
     * @return canWithdraw If anyone can withdraw now
     */
